    ...
    public class DragAndDrop : MonoBehaviour
    {
        ...
        private void CheckObjectWithTag_Joystick(bool b_PuzzleRefPosition)
        {
            if (ReticuleJoystick)
            {
                ...
                RaycastHit hit2;
                if (Physics.Raycast(ray, out hit2, 100, myLayer2))
                {
                    if (hit2.transform.gameObject.CompareTag("PuzzleRefPosition"))
                    {
                        if (hit2.transform.gameObject.GetComponent<SpriteRenderer>())
                        {
        ...
        private void CheckObjectWithTag_Keyboard(bool b_PuzzleRefPosition, bool b_Wait)
        {
            if (ReticuleJoystick)
            {
                ...
                if (Physics.Raycast(ray, out hit, 100, myLayer))
                {
                    if (hit.transform.gameObject.CompareTag("PuzzleObject") && !b_ValidationButtonPressed)
                    {
                ...
                for (int i = 0; i < Input.touchCount; ++i)
                {
                    ...
                    if (fingerNum == i)
                    {
                        ...
                        if (Input.GetTouch(i).phase == TouchPhase.Moved && b_ValidationButtonPressed)
                        {
                            ...
                            RaycastHit hit3;
                            if (Physics.Raycast(ray2, out hit3, 100, myLayer))
                            {
