// Description : w_Feedback.cs : This script is used to create a window tab that allow to add or remove entry (only a title)
#if (UNITY_EDITOR)
using UnityEngine;
...
public class w_Feedback : EditorWindow
{
	public static w_Feedback 		instance = null;              //Static instance of GameManager which allows it to be accessed by any other script.
    ...
    [MenuItem("Tools/AP/Datas Managers/Feedback (w_Feedback)",false,3)]
	...
	void OnEnable () {
		...
		string objectPath = "Assets/AP/Assets/Resources/" + _datasProjectManager.currentDatasProjectFolder + "/TextList/wFeedback.asset";
...
//--> Window description
		...
		EditorGUILayout.BeginVertical(style_Purple);
			EditorGUILayout.HelpBox ("Window Tab : Feedback Texts for Interactable Objects (door, drawer ...)",MessageType.Info);
...
// --> Display Selected ID 
	private void displaySelectedID(GUIStyle style_Orange,GUIStyle style_Blue,GUIStyle style_Yellow_01,int idEditorSize,int titleEditorSize,int textEditorSize,int value){
		...
		for (var i = value; i < value+1; i++) {
			...
			EditorGUILayout.LabelField ("Show in Scene Tab : ", GUILayout.Width (120));
			//--> visualize rentry in the scebe view
			Rect r = GUILayoutUtility.GetLastRect ();

			if (GUI.Button (new Rect (idEditorSize + 170, r.y, 18, 18), eye, GUIStyle.none)) {
...
//> ID Title
            ...
            /*
			EditorGUILayout.LabelField ("");
			if (GUILayout.Button ("Apply to object selected in the Hierarchy")) {
				ApplyPropertiesToGameObject (value);
			}
			EditorGUILayout.LabelField ("");
*/
			EditorGUILayout.EndVertical ();
