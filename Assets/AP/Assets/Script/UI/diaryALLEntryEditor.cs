...
[CustomEditor(typeof(diaryALLEntry))]
public class diaryALLEntryEditor : Editor {
	SerializedProperty			SeeInspector;											// use to draw default Inspector
	SerializedProperty 			diaryButtonDesktop;
	SerializedProperty 			diaryButtonJoystick;
	...
	void OnEnable () {
		...
		diaryButtonDesktop			= serializedObject.FindProperty ("diaryButtonDesktop");
		diaryButtonJoystick			= serializedObject.FindProperty ("diaryButtonJoystick");

		diaryALLEntry myScript = (diaryALLEntry)target; 
	...
	public override void OnInspectorGUI()
	{
		...
		if (objCanvasInput) {
			...
			diaryButtonDesktop.intValue = EditorGUILayout.Popup (diaryButtonDesktop.intValue, s_inputListKeyboardButton.ToArray ());
			EditorGUILayout.EndHorizontal ();

			EditorGUILayout.BeginHorizontal ();
			GUILayout.Label ("Diary Button Joystick : ", GUILayout.Width (150));
			diaryButtonJoystick.intValue = EditorGUILayout.Popup (diaryButtonJoystick.intValue, s_inputListJoystickButton.ToArray ());
			EditorGUILayout.EndHorizontal ();
		} else {
