//Description : reticuleGraphicRaycaster : custom editor for reticuleGraphicRaycaster
#if (UNITY_EDITOR)
using UnityEngine;
...
[CustomEditor(typeof(reticuleGraphicRaycaster))]
public class reticuleGraphicRaycasterEditor : Editor {
	...
	void OnEnable () {
		...
		reticuleGraphicRaycaster myScript = (reticuleGraphicRaycaster)target; 
	...
	public override void OnInspectorGUI()
	{
		...
		if (myScript.editorInfoCase == 0)
			EditorGUILayout.HelpBox ("This script is used to detect pressed button with the reticule in the center of the screen (Keyboard and joystick)", MessageType.Info);
		else if(myScript.editorInfoCase == 1)
			EditorGUILayout.HelpBox ("This script is used to detect pressed button with the fake Joystick mouse", MessageType.Info);
		...
		EditorGUILayout.BeginVertical (style_Yellow_01);
			EditorGUILayout.HelpBox ("The Reticule color when an action is available", MessageType.Info);

			EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Default Button Color : ", GUILayout.Width (150));
				EditorGUILayout.PropertyField(DefaultColor, new GUIContent (""), GUILayout.Width (100));
			EditorGUILayout.EndHorizontal ();

			EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Selected Button Color : ", GUILayout.Width (150));
				EditorGUILayout.PropertyField(SelectedColor, new GUIContent (""), GUILayout.Width (100));
			EditorGUILayout.EndHorizontal ();
